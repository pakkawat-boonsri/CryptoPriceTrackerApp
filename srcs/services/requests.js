...
export const getCoinMarketChart = async (coinId, selectedRange) => {
  try {
    var url = `https://api.coingecko.com/api/v3/coins/${coinId}/market_chart?vs_currency=usd&days=${selectedRange}&interval=hourly`;
    var res = await axios.get(url);
    return await res.data;
  } catch (e) {
    console.log(e);
...
export const getWatchlistedCoinData = async (coinId, pageNumber = 1) => {
  try {
    const url = `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${coinId}&order=market_cap_desc&per_page=50&page=${pageNumber}&sparkline=false&price_change_percentage=24h&locale=en`;
